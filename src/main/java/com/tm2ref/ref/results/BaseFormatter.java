...
public class BaseFormatter {
    ...
    /**
     * out[0] = adminUser.getFullname(); out[1] = rc.getOrg().getName(); out[2]
     * = rc.getUser().getFullname(); out[3] = rc.getJobTitle(); out[4] = null;
     * //rc.getCandidateStartUrl(); out[5] = reminder ?
     * MessageFactory.getStringMessage( locale, "g.REMINDERC" ) + " " : "";
     *
     * out[6] = getRcCheckTypeName( rc ); out[7] = Float.toString(
     * rc.getPercentComplete() ); out[8] = Float.toString( rc.getOverallScore()
     * ); out[9] = rc.getResultsViewUrl();
     *
     * RATER out[15] = rater.getUser().getFullname(); out[16] =
     * rater.getRaterStartUrl(); out[18] = Float.toString(
     * rater.getOverallScore() );
     *
     * CUSTOM coreParams[20] = candidateName; coreParams[21] = raterName;
    ...
    public Object[] getStandardHeaderSection(boolean tog, String introLangKey)
    {
        ...
        {
            // title Row
            label = lmsg("g.RSRefInfo", new String[]
            {
                rcCheckTypeName
            });
    ...
    public Object[] getStandardTopItemsTable(boolean tog, boolean high) throws Exception
    {
        ...
        // title Row
        sb.append(getRowTitle(rowStyleHdr, lmsg(titleKey, new String[]
        {
            topBotSrcTypeName
        }), null, null, null));
    ...
    public Object[] getStandardRatersSection(boolean tog)
    {
        ...
        for (RcRater rater : rc.getRcRaterList())
        {
            ...
            if (rater.getRcRaterStatusType().getIsComplete())
                ...
                statusStr += "<br />" + lmsg("g.RSLastUpdateX", new String[]
                {
                    I18nUtils.getFormattedDateTimeShort(locale, rater.getCompleteDate(), timeZone)
                });

            if (rater.getHasIpLocationData())
                statusStr += "<br />" + lmsg("g.RCIpLocDataX", new String[]
                {
                    rater.getIpLocationData()
                });
    ...
    public Object[] getStandardGenAISection(boolean tog) throws Exception
    {
        ...
        for (AiMetaScore metaScore : rc.getMetaScoreList())
        {
            ...
            scoreText += "<br /><br />" + lmsg("g.AiMetaScrCalcDateX", new String[]
            {
                lastUpdate
            });
    ...
    public Object[] getStandardCompetencySummarySection(boolean tog)
    {
        ...
        int rows = 0;
        String scoreStr, scoreStrCandidate, idealScoreStr, aiScoreStr;
        String descripStr;
        ...
        while (iter.hasNext())
        {
            ...
            aiScoreStr = rcw.getHasCandidateAiScoresToShow(candidateRaterId) ? getAiScoresTable(rcw.getAiScoresCandidate(candidateRaterId), true) : "";
            ...
            if( !aiScoreStr.isBlank() ) 
            sb.append("<tr " + style + "><td style=\"width:60%\">" + aiScoreStr + "</td><td colspan=\"3\"></td></tr>\n");
    ...
    protected String getRaterResponseTableForItem(RcItemWrapper rciw)
    {
        ...
        for (RcRating rating : rciw.getRcRatingList())
        {
            ...
            {
                candidateUploadStr = "<p>" + lmsg("g.UploadedFile") + ": "
                        + "<a href=\"" + rating.getCandidateRcUploadedUserFile().getReportingFileDownloadUrl() + "\" target=\"_blank\" title=\"" + lmsg("g.DownloadUploadedFile") + "\">" + StringUtils.replaceStandardEntities(rating.getCandidateRcUploadedUserFile().getInitialFilename()) + "</a></p>\n";
            }
            commentStr = rating.getText() == null || rating.getText().isBlank() ? "" : StringUtils.replaceStandardEntities(rating.getText());

            // there is a summary str.
            if (rating.getSummary() != null && !rating.getSummary().isBlank())
                commentStr = "<b>" + lmsg("g.SummaryAI") + ":</b> " + StringUtils.replaceStandardEntities(rating.getSummary()) + (commentStr.isBlank() ? "" : "<br /><br />" + lmsg("g.FullResponse") + ": " + commentStr);
            ...
            aiScoreStr = "";
            if (rating.getAiScoresStatusTypeId() == EssayScoreStatusType.SCORECOMPLETE.getEssayScoreStatusTypeId() && rating.getScore2()>0)
                aiScoreStr = getAiScoresTable( rating.getScoresArray(), false );
            ...
            if (hasAv)
                sb.append("<tr " + style + "><td style=\"width:20%;font-size:11pt\">" + lastname + "</td><td style=\"width:7%;text-align:center;font-size:11pt\">" + scoreStr + "</td><td style=\"width:73%;font-size:11pt;text-align:left\">" + selectedRespStr + commentStr + subtext + candidateUploadStr + aiScoreStr + "</td><td style=\"padding:4px\">" + avPlaybackStr + "</td></tr>\n");
            else
                sb.append("<tr " + style + "><td style=\"width:20%;font-size:11pt\">" + lastname + "</td><td style=\"width:7%;text-align:center;font-size:11pt\">" + scoreStr + "</td><td style=\"width:73%;font-size:11pt;text-align:left\">" + selectedRespStr + commentStr + subtext + candidateUploadStr + aiScoreStr + "</td></tr>\n");
    ...
    protected String getAiScoresTable( float[] aiScoresArray, boolean forCompetency )
    {
        StringBuilder aisb = new StringBuilder();
        if( aiScoresArray[2]<=0)
            return aisb.toString();
        
        RcRatingScaleType st = rc.getRcScript().getRcRatingScaleType();

        aisb = new StringBuilder();
        aisb.append("<div><table cellpadding=\"2\" style=\"margin-left:0;width:100%\">\n");
        // aisb.append("<tr><td>" + lmsg("emst.overallai") + ":</td><td>" + I18nUtils.getFormattedNumber(locale, st.convertFrom100Score(rating.getScore2()), 1) + " (" + lmsg("g.ConfidenceX", new String[]{I18nUtils.getFormattedNumber(locale, rating.getScore3()*100, 0)}) + ")</td></tr>\n");
        aisb.append("<tr><td style=\"font-size:0.9rem;font-style:italic\">" + lmsg("emst.overallai" + (forCompetency ? ".comp" : "")) + ":</td><td style=\"font-size:0.9rem;font-style:italic\">" + I18nUtils.getFormattedNumber(locale, st.convertFrom100Score(aiScoresArray[2]), 1) + "</td></tr>\n");
        if (aiScoresArray[12] > 0)
            aisb.append("<tr><td style=\"font-size:0.9rem;font-style:italic\">" + lmsg("emst.clarity" + (forCompetency ? ".comp" : "")) + ":</td><td style=\"font-size:0.9rem;font-style:italic\">" + I18nUtils.getFormattedNumber(locale, st.convertFrom100Score(aiScoresArray[12]), 1) + "</td></tr>\n");

        if (aiScoresArray[13] > 0)
            aisb.append("<tr><td style=\"font-size:0.9rem;font-style:italic\">" + lmsg("emst.argument" + (forCompetency ? ".comp" : "")) + ":</td><td style=\"font-size:0.9rem;font-style:italic\">" + I18nUtils.getFormattedNumber(locale, st.convertFrom100Score(aiScoresArray[13]), 1) + "</td></tr>\n");

        if (aiScoresArray[15] > 0)
            aisb.append("<tr><td style=\"font-size:0.9rem;font-style:italic\">" + lmsg("emst.ideal" + (forCompetency ? ".comp" : "")) + ":</td><td style=\"font-size:0.9rem;font-style:italic\">" + I18nUtils.getFormattedNumber(locale, st.convertFrom100Score(aiScoresArray[15]), 1) + "</td></tr>\n");

        aisb.append("</table></div>\n");
       return aisb.toString();
    }
    ...
    protected String getRcItemHistogram(RcCompetencyWrapper rcw, RcItemWrapper rciw)
    {
        ...
        for (RcHistogramRow row : h.getRowList())
        {
            ...
            sb.append("<tr><td style=\"width:26%\">" + lmsg(row.getLangKey() + ".count", new String[]
            {
                Integer.toString(row.getCount()), row.getOtherNameIfPresent()
            }) + "</td><td style=\"width:7%;text-align:center\">" + I18nUtils.getFormattedNumber(locale, row.getAvgScore(), scrDigits) + "</td><td style=\"width:67%\">");
